...
def postprocessCSV(directory, verbose):
    ...
    if CONFIG_YAML.postprocessing is not None:
        ...
        print(f"No postprocessing config found in config.yaml, using default config, see postprocess_data.py for details.")
        ...
        for file in iterable:
            ...
            if filename.endswith(".csv"):
                ...
                if not filename.startswith("tPose"):
                    postprocess(os.path.join(directory, filename), os.path.join(out_path, filename),postprocessing_config, verbose)
